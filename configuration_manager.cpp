...
int Configuration_Manager::init_scheduler() {
	...
	for (unsigned int i = 0; i < this->camera_indexes_in_use.size(); i++)
	{
		...
		{
			cout << "Error: Configuration_Manager::init_scheduler() tried to initialize camera that is already in use. \n";
			return -1;
		}
	}
...
int Configuration_Manager::scheduler_manager() {
	...
	for (int i = 0; i < this->scheduler_list.size(); i++)
	{
		cout << "-Scheduler_" << i << endl;
	}
	cout << "\n Input: ";
	cin >> selection;
	cout << "\n\n";
	...
	while (1)
	{
		...
		{
		case('1'):
			this->configure_scheduler(selected_scheduler);
			break;

		case('2'):
			this->configure_events(selected_scheduler);
			break;

		case('3'):
			this->configure_image_processor(selected_scheduler);
			break;

		case('4'):
			this->configure_response_module(selected_scheduler);
			break;
...
int Configuration_Manager::configure_scheduler(Scheduler *input_sched) {
	...
	while (1)
	{
		...
		{
			case('1'):
				cout << "Camera_Index= " << input_sched->get_camera_index() << "\n";
				break;

			case('2'):
				cout << "Save_Directory= " << input_sched->get_save_directory() << "\n";
				break;
			...
			case('4'):
				cout << "Scheduler_Resolution= " << input_sched->get_scheduler_resolution() << "\n";
				break;
...
int Configuration_Manager::configure_events(Scheduler *input_sched) {
	...
	while (1)
	{
		...
		{
			case('1'):
				input_sched->print_event_schedule();
				break;

			case('2'):
				input_sched->create_new_event();
				break;

			case('3'):
				input_sched->delete_existing_event();
				break;
...
int Configuration_Manager::configure_image_processor(Scheduler *input_sched) {
	...
	while (1)
	{
		...
		{
			case('1'):
				cout << "Cascade_Path= " << temp_proc->cascade_path << endl;
				break;
			...
			case('3'):
				cout << "Primary_Cascade_Name= " << temp_proc->cascade_name << endl;
				break;
			...
			case('5'):
				cout << "Secondary_Cascade_Name= " << temp_proc->nested_cascade_name << endl;
				break;
...
int Configuration_Manager::configure_response_module(Scheduler *input_sched) {
	...
	while(1)
	{
		...
		{
			case('1'):
				cout << temp_resp->get_mail_destination_address() << endl;
				break;
...
int Configuration_Manager::config_manager_mainloop() {
...
Scheduler* Configuration_Manager::get_scheduler(int scheduler_index) {
	...
	cout << "Configuration_Manager::get_scheduler() tried to access Configuration_Manager::scheduler_list out of bounds. \n";
	return NULL;
}

int Configuration_Manager::get_scheduler_list_size() {

	return this->scheduler_list.size();
}

int Configuration_Manager::select_camera_feed() {
...
int Configuration_Manager::display_all_feeds() {
